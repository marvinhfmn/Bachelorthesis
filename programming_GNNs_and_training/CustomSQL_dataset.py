...
class CustomSQLiteDataset(Dataset):
    """Pytorch dataset for reading data from SQLite databases. Also writes NULL entries from database into graph as '0'"""
    ...
    def query_table(
        ...
        if isinstance(columns, list):
            ...
            # columns = ", ".join([f"IFNULL({col}, 0) AS {col}" for col in columns])
    ...
    def _establish_connection(self, i: int) -> "CustomSQLiteDataset":
    ...
    # @TODO: Is it necessary to return anything here?
    def _close_connection(self) -> "CustomSQLiteDataset":
